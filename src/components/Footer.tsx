                ...
                <div className="grid gap-12 md:grid-cols-4">
                    <div className="md:col-span-2">
                        ...
                        <p className="max-w-sm text-sm leading-7 text-white/50">
                            A destination for nightlife, music, creativity, culture and the people who make Bradford
                            move.
                        </p>
                    </div>
                ...
                <div className="mt-12 grid gap-3 text-sm text-white/65">
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2">
                        {bookingLinks.map(([to, label]) => (
                            <Link key={to} to={to} className="hover:text-[#d8b982]">
                                {label}
                            </Link>
                        ))}
                        <Link to="/waitlist" className="hover:text-[#d8b982]">
                            Join VIP Waitlist
                        </Link>
                    </div>
                </div>
                ...
                <div className="mt-12 flex flex-col justify-between gap-4 border-t border-white/10 pt-6 text-[10px] uppercase tracking-[.15em] text-white/35 md:flex-row">
                    ...
                    <div className="flex flex-wrap gap-x-5 gap-y-2">
                        {legalLinks.map(([to, label]) => (
                            <Link key={to} to={to} className="hover:text-white/60">
                                {label}
                            </Link>
                        ))}
                    </div>
